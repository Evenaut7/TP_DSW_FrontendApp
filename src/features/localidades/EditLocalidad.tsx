...
import { ChevronLeft, RotateCcw, Save, Plus, Pencil, CheckCircle, AlertCircle } from 'lucide-react';
...
import CreatorPDIModal from '@/features/creator/CreatorPDIModal';
import { useEditLocalidad } from './useEditLocalidad';
...
export default function EditLocalidad() {
  ...
  // ── Guards ──
  if (loadingLocalidad) return <PantallaDeCarga mensaje="Cargando localidad..." />;
  if (loadingAuth) return <PantallaDeCarga mensaje="Verificando acceso..." />;
  if (errorAuth) return <p className="text-center mt-4 text-red-500">{errorAuth}</p>;
  if (isAdmin === false) return <p className="text-center mt-4 text-yellow-500">No podés acceder a esta página</p>;
  ...
  const previewUrl = imagenFile ? URL.createObjectURL(imagenFile) : `${API_BASE_URL}/public/${localidad.imagen}`;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="w-full bg-gradient-to-br from-sky-50 via-blue-100 to-slate-50 dark:from-slate-900 dark:via-primary/30 dark:to-slate-800 pt-32 pb-14">
        <div className="max-w-6xl mx-auto px-4">
          ...
          >
            <ChevronLeft className="w-4 h-4" /> Volver a provincias
          </button>
            ...
            <div className="flex flex-col md:flex-row md:items-start gap-8 md:gap-12">
              <div className="flex-1 space-y-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500 flex items-center gap-1.5">
                    <Pencil className="w-3 h-3" /> Modificar provincia
                  </span>
                  <select
              ...
              <div className="w-full md:w-80 lg:w-96 flex-shrink-0">
                <div className="relative rounded-2xl overflow-hidden shadow-xl aspect-[4/3] ring-1 ring-slate-200 dark:ring-white/10">
                  <img src={previewUrl} alt={localidad.nombre} className="w-full h-full object-cover" />
        ...
        <div className="max-w-6xl mx-auto px-4 space-y-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              {localidad.puntosDeInteres?.length ?? 0} PDIs en {localidad.nombre}
            </p>
          </div>
