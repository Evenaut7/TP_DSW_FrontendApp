...
import { useApiGetById, addEventoToAgenda, removeEventoFromAgenda, checkEventoAgendado } from '@/utils/api';
...
const ListadoEventos: React.FC<ListadoEventosProps> = ({ pdiId, tagIds = [] }) => {
  const { user } = useUser();
  ...
  const [modalConfig, setModalConfig] = useState({ success: true, message: '' });
  ...
  useEffect(() => {
    ...
    const checkAgendados = async () => {
      const checks = await Promise.all(
        data.eventos.map(async (evento) => {
          try {
            ...
            return {
              id: evento.id,
              agendado: result.success && (result.data as { estaAgendado: boolean })?.estaAgendado,
            };
          } catch {
      ...
      const agendados = new Set(checks.filter((c) => c.agendado).map((c) => c.id));
  ...
  const handleToggleAgenda = async (eventoId: number) => {
    if (!user) {
      setModalConfig({ success: false, message: 'Debes iniciar sesión para agendar eventos' });
      setShowModal(true);
      return;
    }
    ...
    try {
      ...
      const result = isAgendado ? await removeEventoFromAgenda(eventoId) : await addEventoToAgenda(eventoId);

      if (result.success) {
        setEventosAgendados((prev) => {
          const newSet = new Set(prev);
          isAgendado ? newSet.delete(eventoId) : newSet.add(eventoId);
          return newSet;
        });
        setModalConfig({
          success: true,
          message: isAgendado ? 'Evento removido de tu agenda' : 'Evento agregado a tu agenda',
        });
      } else {
        setModalConfig({ success: false, message: result.error || 'Error al actualizar la agenda' });
      }
      setShowModal(true);
    } catch {
      setModalConfig({ success: false, message: 'Error al procesar la solicitud' });
      setShowModal(true);
    } finally {
  ...
  if (loading) return <p className="text-slate-400 dark:text-slate-500 text-sm">Cargando eventos...</p>;
  if (error) return <p className="text-red-500 text-sm">{error}</p>;
  if (!data || data.eventos.length === 0)
    return <p className="text-slate-400 dark:text-slate-500 text-sm">No hay eventos disponibles.</p>;

  // Filtrar por tags si hay alguno seleccionado
  ...
  return (
    <>
      {eventosFiltrados.length === 0 ? (
        <p className="text-slate-400 dark:text-slate-500 text-sm">No hay eventos para los filtros seleccionados.</p>
      ) : (
